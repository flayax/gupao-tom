...
public class Singleton4 {
	//1、先声明一个静态内部类，Singleton4类加载时已被初始化，且只有一份
	//private 私有的保证别人不能修改
	//static 保证全局唯一
	private static class LazyHolder {
		//final 为了防止内部误操作，代理模式，GgLib的代理模式，代理类为被代理类的子类，可以对其进行修改，如在源代码之前或之后修改对象的引用
		private static final Singleton4 INSTANCE = new Singleton4();
	}
	//2、将默认构造方法私有化
	private Singleton4 (){}
	//若不用private的构造方法，则相当于有一个默认的public的无参的构造方法，就意味着在代码中随时都可以new出来
	...
	public static final Singleton4 getInstance() {  
		...
		//方法中实现逻辑需要分配内存，也是调用时才分配的，比直接使用静态块节省内存空间
